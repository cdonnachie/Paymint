...
class _UtxoDetailViewState extends State<UtxoDetailView> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < ravencoinLiteService.allOutputs.length; i++) {
      if (ravencoinLiteService.allOutputs[i].txid == _utxoObj.txid) {
        _utxoObject = ravencoinLiteService.allOutputs[i];
      }
    }
    return Scaffold(
      appBar: AppBar(
        ...
        child: ListView(
          children: <Widget>[
            ...
            ListTile(
              ...
              trailing: Text(
                (_utxoObject.value / 100000000).toString() + ' RVL',
                style: TextStyle(color: Colors.white),
              ),
              onTap: () {},
            ),
            ...
            ListTile(
              ...
              onTap: () async {
                if (_utxoObject.blocked == true) {
                  final service = Provider.of<RavencoinLiteService>(context);
                  ...
                  final List blockedCopyWithoutTxid = [];
      ...
      actions: <Widget>[
        ElevatedButton(
          ...
          onPressed: () async {
            ...
            final RavencoinLiteService ravencoinLiteService =
                Provider.of<RavencoinLiteService>(context);
            ravencoinLiteService.renameOutput(txid, textEditingController.text);
            Navigator.pop(context);
          },
        ),
        ElevatedButton(
...
class PurchaseListTile extends StatelessWidget {
  ...
  final String purchaseAmount; // Denominated in RVL
  final String
...
class __SendDetailsPageState extends State<_SendDetailsPage> {
  ...
  @override
  void initState() {
    this.viewDenomination = 0; // 0 == Satoshis & 1 == RVL denomination
    super.initState();
  }
  ...
  buildAmount() {
    if (viewDenomination == 0) {
      ...
      return (widget._tx.amount / 100000000).toString() + ' RVL';
    }
  }
  ...
  buildFeeAmount() {
    if (viewDenomination == 0) {
      ...
      return (widget._tx.fees / 100000000).toString() + ' RVL';
    }
  }
